...
final class NonceGenerator {
  ...
  public static String getNonceAsBase64EncodedString(final int randomByteAmount) {
    if (randomByteAmount < NONCE_BYTE_AMOUNT_MIN || randomByteAmount > NONCE_BYTE_AMOUNT_MAX) {
      throw new GmsCryptoException(
    ...
    return Base64.getEncoder().encodeToString(randomArray);
  }
  ...
  public static String getNonceAsHex(final int strlen) {
    ...
    if (strlen % 2 != 0) {
      throw new GmsCryptoException("Requested string length is expected to be even.");
    }
    final byte[] randomArray = randomBytes(strlen / 2);
    return Hex.toHexString(randomArray);
  }

  private static byte[] randomBytes(final int numberOfBytes) {
    final byte[] bytes = new byte[numberOfBytes];
    try {
      SecureRandom.getInstanceStrong().nextBytes(bytes);
    } catch (final NoSuchAlgorithmException e) {
      throw new GmsCryptoException("Error while generating random bytes", e);
    }
    return bytes;
  }
}
